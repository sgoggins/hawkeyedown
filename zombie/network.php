...
<html>
<form action = "network.php" method = "POST" name = "form1">
...
if (isset($_POST['jobs']) ){
$jobs = $_POST['jobs'];
$title = "Direct Message Network for Jobs ".$jobs ;
...
mysql_select_db('twitteranalyze', $con);
  ...
  die('Could not connect: ' . mysql_error());
   ...
   $rs = mysql_query("select from_user,to_user, from_user_id_str, to_user_id_str, count(*) as counter from tweet_analyze
	...
	while($row = mysql_fetch_assoc($rs))
	{
		...
		for($i =0; $i < count($users); $i++){
			if ($row['from_user_id_str'] == $users[$i]['id']){
				$u1 = true;
			}
			if ($row['to_user_id_str'] == $users[$i]['id']){
				$u2 = true;
			}
		}
		if($u1==false){
			$u = array('id'=>$row['from_user_id_str'],'name'=>$row['from_user']);
			$users[] = $u;
		}
		if($u2==false){
			$u = array('id'=>$row['to_user_id_str'],'name'=>$row['to_user']);
			$users[] = $u;
		}
